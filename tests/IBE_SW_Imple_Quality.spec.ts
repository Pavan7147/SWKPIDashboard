...
test('IntelliBridge Enterprise Validation for Software Implementation Quality', async ({page}, testInfo) => {
  ...
  await testInfo.attach("screenshot",
    ...
    contentType: 'IntelliBridge Enterprise_Software Implementation Quality/png' 
    }
    )
  await page1.locator('div').filter({ hasText: /^ReleaseSelect\.\.\.$/ }).locator('svg').click();
  await page1.getByRole('option', { name: 'B.21', exact: true }).click();
  await page1.getByRole('button', { name: 'Submit' }).click();
  await page1.waitForTimeout (20000);
  await page1.getByLabel('Formula').click();
  await testInfo.attach("screenshot",
    { body:await page.screenshot(), 
    contentType: 'IntelliBridge Enterprise_Software Implementation Quality_Release/png' 
    }
    ) 
  console.log("-----------------------------------------------------------------------");
  console.log("IntelliBridge Enterprise Validation for Software Implementation Quality is completed successfully");
  console.log("-----------------------------------------------------------------------");
  await browser.close();
  });