...
test('ISCV Validation for Software Implementation Quality', async ({page}, testInfo) => {
  ...
  await page1.getByRole('option', { name: 'ISCV' }).click();
  await page1.waitForTimeout (20000);
  await testInfo.attach("screenshot",
    { body:await page.screenshot(), 
    contentType: 'ISCV_Software Implementation Quality/png' 
    }
    )
  await page1.locator('div').filter({ hasText: /^ReleaseSelect\.\.\.$/ }).locator('svg').click();
  await page1.getByRole('option', { name: 'ISCV 8.0.0.2', exact: true }).click();
  await page1.getByRole('button', { name: 'Submit' }).click();
  await page1.waitForTimeout (20000);
  await page1.getByLabel('Formula').click();
  await testInfo.attach("screenshot",
    { body:await page.screenshot(), 
    contentType: 'ISCV_Software Implementation Quality_Release/png' 
    }
    ) 
  console.log("-----------------------------------------------------------------------");
  console.log("ISCV Validation for Software Implementation Quality is completed successfully");
  console.log("-----------------------------------------------------------------------");
  await browser.close();
  });