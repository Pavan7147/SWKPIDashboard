...
test('VueCloud* Validation for Defect Escape Rate', async ({page}, testInfo) => {
  ...
  await testInfo.attach("screenshot",
    ...
    contentType: 'VueCloud*_Defect_Escape_Rate/png' 
    }
    )
  await page1.locator('div').filter({ hasText: /^ReleaseSelect\.\.\.$/ }).locator('svg').click();
  await page1.getByRole('option', { name: '12.2.8.420' }).click();
  await page1.getByRole('button', { name: 'Submit' }).click();
  await page1.waitForTimeout (20000);
  await testInfo.attach("screenshot",
    { body:await page.screenshot(), 
    contentType: 'VueCloud*_Defect_Escape_Rate_Release/png' 
    }
    )
  await page1.locator('.main-container').click();
  await page1.getByLabel('Formula').click(); 
  console.log("-----------------------------------------------------------------------");
  console.log("VueCloud* Validation for Defect Escape Rate is completed successfully");
  console.log("-----------------------------------------------------------------------");
  await browser.close();
  });