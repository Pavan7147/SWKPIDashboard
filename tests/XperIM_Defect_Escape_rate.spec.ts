...
test('XperIM Validation for Defect Escape Rate', async ({page}, testInfo) => {
  ...
  await testInfo.attach("screenshot",
    ...
    contentType: 'XperIM_Defect_Escape_Rate/png' 
    }
    )
  await page1.locator('div').filter({ hasText: /^ReleaseSelect\.\.\.$/ }).locator('svg').click();
  await page1.getByRole('option', { name: '2.4', exact: true }).click();
  await page1.getByRole('button', { name: 'Submit' }).click();
  await page1.waitForTimeout (20000);
  await testInfo.attach("screenshot",
    { body:await page.screenshot(), 
    contentType: 'XperIM_Defect_Escape_Rate_Release/png' 
    }
    )
  await page1.locator('.main-container').click();
  await page1.getByLabel('Formula').click(); 
  console.log("-----------------------------------------------------------------------");
  console.log("XperIM Validation for Defect Escape Rate is completed successfully");
  console.log("-----------------------------------------------------------------------");
  await browser.close();
  });